...
namespace HabitsApp.Application.Behaviors;
    ...
    where TRequest : class, IRequest<TResponse>
{
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        if(!_validators.Any())
        {
            return await next(); // eğer validasyon yoksa devam et
        }
        var context = new ValidationContext<TRequest>(request); // validasyon context olustur
